    ...
    if(taken)
    {
        ...
        if(bufferPC == IF2ID.PC)
        {
            ...
            if((CURRENT_STATE.PC&(~31))!= (bufferPC&(~31))){
             ...
             //if(CURRENT_STATE.PC == bufferPC){
               // printf("buffer counting because CURR.PC == bufferPC\n");
            bufferCount = 1;
            IF2ID.writeEnable = 0;
            //}
        ...
        switch (ID2EX.opcode) {
            ...
            case 5:
                ...
                if(bufferPC == IF2ID.PC)
                {
                    ...
                    printf("   is really a taken branch, but we predicted incorrectly B\n");

                    printf("PC = %x, bufferPC = %x\n", CURRENT_STATE.PC, bufferPC);
                    if((CURRENT_STATE.PC&(~31))!= (bufferPC&(~31))){
                        printf("cancelling miss because CURR.PC is in same block as new bufferPC\n");
                        IF_Stall = 0;
                         bufferCount = 1;
                        IF2ID.writeEnable = 0;
                    }
                    ...
                    killFetch();
                    // printf("inside B, setting CURREN_STATE = %x --> %x\n", CURRENT_STATE.PC, bufferPC);
                    CURRENT_STATE.PC = bufferPC;
                    bp_update(ID2EX.PC, bufferPC, false, true);
                }
                break;
